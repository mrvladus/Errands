...
static void errands_month_chooser_init(ErrandsMonthChooser *self) {
  ...
  g_object_set(box, "margin-start", 12, "margin-end", 12, "margin-top", 6, "margin-bottom", 6,
               NULL);
  gtk_box_append(GTK_BOX(box), g_object_new(GTK_TYPE_LABEL, "label", _("Months"), "halign",
...
const char *errands_month_chooser_get_months(ErrandsMonthChooser *mc) {
  ...
  if (!str_eq_c(&by_month, "")) {
    str_prepend(&by_month, "BYMONTH=");
    str_append(&by_month, ";");
  }
