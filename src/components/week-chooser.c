...
static void errands_week_chooser_init(ErrandsWeekChooser *self) {
  ...
  g_object_set(box, "margin-start", 12, "margin-end", 12, "margin-top", 6, "margin-bottom", 6,
               NULL);
  gtk_box_append(GTK_BOX(box), g_object_new(GTK_TYPE_LABEL, "label", _("Week Days"), "halign",
...
ErrandsWeekChooser *errands_week_chooser_new() {
  return g_object_new(ERRANDS_TYPE_WEEK_CHOOSER, NULL);
}

const char *errands_week_chooser_get_days(ErrandsWeekChooser *wc) {
  GPtrArray *week_days = get_children(wc->week_box);
  GString *week_days_str = g_string_new("");
  for (int i = 0; i < week_days->len; i++) {
  ...
  g_ptr_array_free(week_days, true);
  ...
  g_string_free(week_days_str, true);
...
void errands_week_chooser_set_days(ErrandsWeekChooser *wc, const char *rrule) {
  ...
  g_ptr_array_free(week_days_btns, true);
}
