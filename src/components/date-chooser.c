...
static void errands_date_chooser_init(ErrandsDateChooser *self) {
  ...
  // Button
  GtkWidget *btn = g_object_new(GTK_TYPE_MENU_BUTTON, "popover", popover, "icon-name",
                                "errands-calendar-symbolic", "valign", GTK_ALIGN_CENTER, NULL);
  ...
  // Reset button
  self->reset_btn = g_object_new(GTK_TYPE_BUTTON, "icon-name", "errands-reset-symbolic",
                                 "tooltip-text", _("Reset"), "valign", GTK_ALIGN_CENTER, NULL);
  gtk_widget_add_css_class(self->reset_btn, "flat");
  g_signal_connect_swapped(self->reset_btn, "clicked", G_CALLBACK(errands_date_chooser_reset),
                           self);
...
ErrandsDateChooser *errands_date_chooser_new() {
  return g_object_new(ERRANDS_TYPE_DATE_CHOOSER, NULL);
}

const char *errands_date_chooser_get_date(ErrandsDateChooser *dc) {
  GDateTime *datetime = gtk_calendar_get_date(GTK_CALENDAR(dc->calendar));
  char *date_str = g_date_time_format(datetime, "%Y%m%d");
  static char date[9];
...
static void on_date_set_cb(ErrandsDateChooser *dc) {
  GDateTime *date = gtk_calendar_get_date(GTK_CALENDAR(dc->calendar));
