...
static void errands_time_chooser_init(ErrandsTimeChooser *self) {
  ...
  g_object_set(afternoon_btn_content, "icon-name", "errands-afternoon-symbolic", "label", "12:00",
               NULL);
  ...
  g_signal_connect_swapped(self->reset_btn, "clicked", G_CALLBACK(errands_time_chooser_reset),
                           self);
  ...
  GtkWidget *btn = g_object_new(
      GTK_TYPE_MENU_BUTTON, "popover", g_object_new(GTK_TYPE_POPOVER, "child", clock_hbox, NULL),
...
static void on_time_preset_cb(GtkButton *btn, const char *h) {
  ErrandsTimeChooser *tc =
      ERRANDS_TIME_CHOOSER(gtk_widget_get_ancestor(GTK_WIDGET(btn), ERRANDS_TYPE_TIME_CHOOSER));
