...
void errands_priority_window_show(ErrandsTask *task) {
  ...
  // Toggle for priority
  const uint8_t priority = task_data_get_priority(task->data);
  if (priority == 0)
    gtk_check_button_set_active(GTK_CHECK_BUTTON(state.priority_window->none_btn), true);
  else if (priority == 1)
    gtk_check_button_set_active(GTK_CHECK_BUTTON(state.priority_window->low_btn), true);
  else if (priority == 5)
    gtk_check_button_set_active(GTK_CHECK_BUTTON(state.priority_window->medium_btn), true);
  else if (priority == 9)
    gtk_check_button_set_active(GTK_CHECK_BUTTON(state.priority_window->high_btn), true);
...
static void on_errands_priority_window_close(ErrandsPriorityWindow *win, gpointer data) {
  ...
  if (task_data_get_priority(win->task->data) != val) {
    task_data_set_priority(win->task->data, val);
  ...
  if (val == 0)
    ...
    gtk_widget_add_css_class(win->task->toolbar->priority_btn, "priority-medium");
