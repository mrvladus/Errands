...
void errands_tags_window_show(ErrandsTask *task) {
  ...
  g_auto(GStrv) tags = list_data_get_all_tags();
  const size_t length = tags ? g_strv_length(tags) : 0;
  // Add rows
...
static void on_errands_tags_window_tag_added(GtkEditable *entry, ErrandsTagsWindow *win) {
  ...
  // Return if exists
  g_auto(GStrv) tags = list_data_get_all_tags();
  if (tags && g_strv_contains((const gchar *const *)tags, tag))
    return;
  // Add tag to current list data
  ListData *data = task_data_get_list(win->task->data);
  list_data_add_tag(data, tag);
...
ErrandsTagsWindowRow *errands_tags_window_row_new(const char *tag) {
  ...
  g_auto(GStrv) tags = task_data_get_tags(state.tags_window->task->data);
...
static void on_errands_tags_window_row_toggle(GtkCheckButton *btn, ErrandsTagsWindowRow *row) {
  ...
  gtk_check_button_get_active(btn) ? task_data_add_tag(data, tag) : task_data_remove_tag(data, tag);
...
static void on_errands_tags_window_row_delete(GtkButton *btn, ErrandsTagsWindowRow *row) {
  ...
  for (size_t i = 0; i < tasks->len; i++)
    errands_task_update_tags(tasks->pdata[i]);
  errands_tags_window_update_ui();
