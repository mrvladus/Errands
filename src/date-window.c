...
void errands_date_window_show(ErrandsTask *task) {
  ...
  LOG("Date window: Show for '%s'", task_data_get_uid(task->data));
  ...
  // Set start date
  const char *start_dt = task_data_get_start(task->data);
  if (start_dt) {
  ...
  const char *due_dt = task_data_get_due(task->data);
  if (due_dt) {
    g_autoptr(GString) d_date = g_string_new(task_data_get_due(task->data));
  ...
  // Setup repeat
  const char *rrule = task_data_get_rrule(task->data);
...
static void on_errands_date_window_close_cb(ErrandsDateWindow *win, gpointer data) {
  ...
  if (start_date_is_set) {
    ...
    task_data_set_start(win->task->data, s_dt->str);
  } else {
    if (start_time_is_set) {
      const char *start_time = errands_time_chooser_get_time(win->start_time_chooser);
  ...
  if (due_date_is_set) {
    ...
    task_data_set_due(win->task->data, d_dt->str);
  } else {
    if (due_time_is_set) {
      const char *due_time = errands_time_chooser_get_time(win->due_time_chooser);
  ...
  if (!adw_expander_row_get_enable_expansion(ADW_EXPANDER_ROW(win->repeat_row)))
    // If not - clean rrule
    task_data_set_rrule(win->task->data, "");
  // Generate new rrule
  // else {
  //   // Get frequency
  ...
  //   //   const char *time = strstr(task_data_get_start(win->task->data), "T");
  //   //   if (time)
  ...
  //     task_data_set_rrule(win->task->data, rrule->str);
  // }
...
static void on_count_changed_cb(AdwSpinRow *row, GParamSpec *param, ErrandsDateWindow *win) {
  ...
  if (value == 0)
    g_object_set(row, "subtitle", _("Repeat forever"), NULL);
  else {
