...
ErrandsTaskListTagsDialogTag *errands_task_list_tags_dialog_tag_new(const char *tag) {
  ...
  g_auto(GStrv) tags =
      errands_data_get_strv(state.main_window->task_list->tags_dialog->current_task->data, DATA_PROP_TAGS);
...
static void on_toggle_cb(ErrandsTaskListTagsDialogTag *self, GtkCheckButton *btn) {
  ...
  TaskData *data = state.main_window->task_list->tags_dialog->current_task->data;
...
static void on_delete_cb(ErrandsTaskListTagsDialogTag *self, GtkButton *btn) {
  ...
  gtk_list_box_remove(GTK_LIST_BOX(state.main_window->task_list->tags_dialog->tags_box), GTK_WIDGET(self));
  ...
  errands_task_list_tags_dialog_update_ui(state.main_window->task_list->tags_dialog);
