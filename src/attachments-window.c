...
static void errands_attachments_window_init(ErrandsAttachmentsWindow *self) {
  ...
  g_signal_connect(add_btn, "clicked",
                   G_CALLBACK(on_errands_attachments_window_attachment_add),
                   NULL);
...
void errands_attachments_window_show(ErrandsTask *task) {
  ...
  for (int i = 0; i < task->data->attachments->len; i++) {
    ...
    gtk_list_box_append(GTK_LIST_BOX(state.attachments_window->list_box),
                        GTK_WIDGET(row));
  }
  errands_attachments_window_update_ui();
  // Show dialog
  adw_dialog_present(ADW_DIALOG(state.attachments_window),
                     GTK_WIDGET(state.main_window));
}

static void errands_attachments_window_update_ui() {
  if (state.attachments_window->task->data->attachments->len > 0) {
    char *len = g_strdup_printf(
        "%d", state.attachments_window->task->data->attachments->len);
    adw_window_title_set_subtitle(
        ADW_WINDOW_TITLE(state.attachments_window->title), len);
    g_free(len);
    gtk_widget_set_visible(state.attachments_window->placeholder, false);
  } else {
    adw_window_title_set_subtitle(
        ADW_WINDOW_TITLE(state.attachments_window->title), "");
...
on_errands_attachments_window_row_delete(GtkButton *btn,
                                         ErrandsAttachmentsWindowRow *row);
static void
on_errands_attachments_window_row_destroy(ErrandsAttachmentsWindowRow *row);

G_DEFINE_TYPE(ErrandsAttachmentsWindowRow, errands_attachments_window_row,
              ADW_TYPE_ACTION_ROW)

static void errands_attachments_window_row_class_init(
    ErrandsAttachmentsWindowRowClass *class) {}
...
errands_attachments_window_row_init(ErrandsAttachmentsWindowRow *self) {
  ...
  g_signal_connect(self->del_btn, "clicked",
                   G_CALLBACK(on_errands_attachments_window_row_delete), self);
...
ErrandsAttachmentsWindowRow *errands_attachments_window_row_new(GFile *file) {
  ErrandsAttachmentsWindowRow *row =
      g_object_new(ERRANDS_TYPE_ATTACHMENTS_WINDOW_ROW, NULL);
  g_signal_connect(row, "destroy",
                   G_CALLBACK(on_errands_attachments_window_row_destroy), NULL);
...
on_errands_attachments_window_row_delete(GtkButton *btn,
  ...
  g_free(path);
  errands_data_write();
  gtk_list_box_remove(GTK_LIST_BOX(state.attachments_window->list_box),
                      GTK_WIDGET(row));
  errands_attachments_window_update_ui();
}
