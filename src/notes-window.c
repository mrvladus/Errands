...
void errands_notes_window_show(TaskData *td) {
  ...
  state.notes_window_uid = td->uid;
  GtkTextBuffer *buf =
      gtk_text_view_get_buffer(GTK_TEXT_VIEW(state.notes_window_source_view));
  gtk_text_buffer_set_text(buf, td->notes, -1);
}

static void on_errands_notes_window_close(AdwDialog *win, gpointer data) {
  TaskData *td = errands_data_get_task(state.notes_window_uid);
  GtkTextBuffer *buf =
      gtk_text_view_get_buffer(GTK_TEXT_VIEW(state.notes_window_source_view));
...
void errands_notes_window_build() {
  ...
  // Source view
  state.notes_window_source_view = gtk_source_view_new();
  g_object_set(state.notes_window_source_view, "top-margin", 6, "bottom-margin",
               6, "right-margin", 6, "left-margin", 6, "wrap-mode",
               GTK_WRAP_WORD, NULL);

  // Scrolled window
  GtkWidget *scrl = gtk_scrolled_window_new();
  gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(scrl),
                                state.notes_window_source_view);
  ...
  g_signal_connect(state.notes_window, "closed",
                   G_CALLBACK(on_errands_notes_window_close), NULL);
}
