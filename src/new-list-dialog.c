...
static void on_response_cb(ErrandsNewListDialog *dialog, gchar *response,
                           gpointer data);

// --- IMPLEMENTATIONS --- //

G_DEFINE_TYPE(ErrandsNewListDialog, errands_new_list_dialog,
              ADW_TYPE_ALERT_DIALOG)

static void
errands_new_list_dialog_class_init(ErrandsNewListDialogClass *class) {}
...
void errands_new_list_dialog_show() {
  adw_dialog_present(ADW_DIALOG(state.new_list_dialog), state.main_window);
...
static void on_entry_changed_cb(AdwEntryRow *entry) {
  adw_alert_dialog_set_response_enabled(
      ADW_ALERT_DIALOG(state.new_list_dialog), "create",
